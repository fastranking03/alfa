...
router.get("/", async (req, res) => {
    try {
        ...
        const [lastProducts] = await connect.query("SELECT * FROM products ORDER BY created_at DESC LIMIT 5");
...
router.post("/place-order", async (req, res) => {
    ...
    try {
        ...
        request.requestBody({
            ...
            application_context: {
                return_url: 'http://alfa-menswear.com/paypal-callback',
                cancel_url: 'http://alfa-menswear.com/cart'
            }
        });
...
router.get("/paypal-callback", async (req, res) => {
    ...
    try {
      ...
      INSERT INTO orders (order_id, user_id, user_name , payer_id , transaction_amount_online , currency , payment_status_online , transaction_id , total_payable, vat, delivery_charges, sub_total, total_mrp, discount_amount, address_id , order_status) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "order placed")
    `;
...
WHERE p.wear_type_bottom_or_top IN ('top', 'bottom', 'suit')
GROUP BY p.id;
...
router.get("/accessories", async (req, res) => {
    ...
    try {
        ...
        const [results] = await connect.query(queryProduct1);
        const [categories] = await connect.query(querycategoryList);
        const [color_list] = await connect.query(colorlist);
        // console.log(results);
...
router.get("/product-detail/:slug/:id", async (req, res) => {
    try {
       ...
       ORDER BY star_rating DESC`,
        ...
        } else if (wearType === "suit") {
            sizeQuery = `SELECT size_34, size_36, size_38, size_40, size_42, size_44, size_46 , size_48, size_50, size_52, size_54, size_56 FROM suits_wear_inventory_with_sizes WHERE product_id = ?`;
        }
            ...
            for (const [size, value] of Object.entries(sizeRows[0])) {
                if (value) {
                    ...
                    if (wearType === "top" && !size.startsWith("size_")) {
                        ...
                        sizes[size.replace("_", " ")] = value;
                    } else if (wearType === "suit" && size.startsWith("size_")) {
                        sizes[size.replace("_", " ")] = value;
                    }
                }
            }
      ...
      WHERE c.user_id = ?`,
        ...
        const cartItemPromises = cartResultAll.map(async (cartItem) => {
           ...
           WHERE product_id = ?`,
             ...
             WHERE product_id = ?`,
                ...
                sizes = sizeRows[0];
            } else if (cartItem.wear_type_bottom_or_top === "suit") {
                const [sizeRows] = await connect.query(
                    `SELECT size_34, size_36, size_38, size_40, size_42, size_44, size_46 ,size_48, size_50, size_52, size_54, size_56  
         FROM suits_wear_inventory_with_sizes 
         WHERE product_id = ?`,
                    [cartItem.product_id]
                );
                sizes = sizeRows[0];
        ...
        cartItems.forEach((item) => {
            ...
            if (selectedSize) {
                ...
                } else if (item.wear_type_bottom_or_top === "bottom" || item.wear_type_bottom_or_top === "suit") {
                    const sizeKey = `size_${selectedSize}`;
                    const sizeKeyLowerCase = sizeKey.toLowerCase();
        ...
        let deliveryFee = 0;
        if (parseFloat(subtotal) < 75) {
            deliveryFee = 25; // Delivery fee if subtotal is less than 75
        }
        //
        // 
        const totalCost = (
        ...
        if (addressCount < 1) {
            return res.redirect("/add-address"); // Use res.redirect, not req.redirect
        }
        ...
        if (!user) {
            ...
            const cartItemPromises = resolvedCartItems.map(async (cartItem) => {
             ...
             WHERE product_id = ?`,
                    ...
                    const [sizeRows] = await connect.query(
                        `SELECT size_34, size_36, size_38, size_40, size_42, size_44, size_46 ,size_48, size_50, size_52, size_54  
             FROM suits_wear_inventory_with_sizes 
             WHERE product_id = ?`,
                        [cartItem.product_id]
                    );
...
router.get("/add-address", (req, res, next) => {
    try {
        ...
        // Render the view with session or default data
...
router.post("/submit-address", async (req, res) => {
    ...
    try {
      ...
      WHERE id = ?
        ...
        // res.location("/checkout").status(302).end();
        res.location("https://www.alfa-menswear.com/checkout").status(302).end();
...
router.post("/add-to-cart", async (req, res) => {
    // console.log("Session User Before Check:", req.session.user);
    const user = req.session.user;
